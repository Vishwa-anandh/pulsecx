...
import { LayoutDashboard, Activity, Terminal, ShieldAlert, Users, Settings, User, PanelLeftClose, PanelLeftOpen, Route, MonitorPlay, Bell, Network, ShieldCheck, Smartphone, BarChart2, Sun, Moon, Search, Shield, Server, FileText } from 'lucide-react';
...
export default function Sidebar({ setIsSearchOpen }) {
  ...
  const adminItems = [
    { name: 'User Management', path: '/settings/users', icon: <Users size={16} /> },
    { name: 'Team Management', path: '/settings/teams', icon: <Shield size={16} /> },
    { name: 'Integrations', path: '/settings/integrations', icon: <Network size={16} /> },
    { name: 'Monitoring Agents', path: '/settings/agents', icon: <Server size={16} /> },
    { name: 'Audit Logs', path: '/settings/audit', icon: <FileText size={16} /> },
  ];

  return (
    <aside className={`sidebar ${isCollapsed ? 'collapsed' : ''}`} style={{ height: '100%', padding: '1rem', display: 'flex', flexDirection: 'column' }}>
      <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'}`} style={{ marginBottom: '2rem', padding: isCollapsed ? '0' : '0 0.5rem' }}>
        <div className="flex items-center gap-2" style={{ overflow: 'hidden' }}>
          <div style={{ width: '28px', height: '28px', borderRadius: '6px', background: 'var(--bg-surface-hover)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--border-color)', flexShrink: 0, transition: 'transform 0.5s ease' }} onMouseEnter={(e) => e.currentTarget.style.transform = 'rotate(10deg) scale(1.05)'} onMouseLeave={(e) => e.currentTarget.style.transform = 'rotate(0deg) scale(1)'}>
            <img src="/favicon.svg" alt="Logo" style={{ width: '18px', height: '18px' }} />
          </div>
          {!isCollapsed && <h2 style={{ margin: 0, fontSize: '1.1rem', letterSpacing: '-0.04em', opacity: 1, transition: 'opacity 0.2s' }}>pulse<span className="text-gradient">CX</span></h2>}
        </div>
        {!isCollapsed && (
      ...
      <div className="sidebar-scroll" style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', display: 'flex', flexDirection: 'column' }}>
        {!isCollapsed && (
          ...
          {navItems.map((item) => (
            <NavLink
              ...
              style={{ 
                ...
                ...(isCollapsed ? { width: '40px', height: '40px', padding: 0, margin: '0.2rem auto' } : {})
              }}
            >
      ...
      <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '1rem', position: 'relative' }}>
        {profileMenuOpen && (
          <div className="animate-fade-in" style={{ position: 'absolute', bottom: '100%', left: '1rem', right: '1rem', background: 'var(--bg-surface)', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-lg)', padding: '0.5rem', boxShadow: 'var(--shadow-lg)', zIndex: 100, marginBottom: '0.5rem' }}>
            <div className="hover-bg-surface-hover" style={{ padding: '0.5rem', borderRadius: 'var(--radius-md)', cursor: 'pointer', fontSize: '0.8125rem' }} onClick={() => setProfileMenuOpen(false)}>Your Profile</div>
            <div className="hover-bg-surface-hover" style={{ padding: '0.5rem', borderRadius: 'var(--radius-md)', cursor: 'pointer', fontSize: '0.8125rem' }} onClick={() => setProfileMenuOpen(false)}>Preferences</div>
        ...
        }} onMouseEnter={(e) => e.currentTarget.style.boxShadow = 'var(--shadow-md)'} onMouseLeave={(e) => e.currentTarget.style.boxShadow = 'var(--shadow-sm)'} data-tooltip={isCollapsed ? "Mark Bennet" : undefined}>
          <div className="flex items-center gap-2" onClick={() => setProfileMenuOpen(!profileMenuOpen)}>
            ...
            {!isCollapsed && (
              <div style={{ display: 'flex', flexDirection: 'column', whiteSpace: 'nowrap' }}>
                <span style={{ fontSize: '0.875rem', fontWeight: '700', color: 'var(--text-primary)' }}>Mark Bennet</span>
              </div>
            )}
          </div>
          
          {!isCollapsed && (
            <div style={{ display: 'flex', alignItems: 'center', background: 'var(--bg-surface-hover)', borderRadius: 'var(--radius-full)', padding: '2px', flexShrink: 0, cursor: 'pointer' }} onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')}>
               <div style={{ padding: '4px', borderRadius: '50%', background: theme === 'light' ? 'var(--nav-active-bg)' : 'transparent', color: theme === 'light' ? 'var(--nav-active-text)' : 'var(--text-muted)', transition: 'all 0.2s' }}><Sun size={12} /></div>
               <div style={{ padding: '4px', borderRadius: '50%', background: theme === 'dark' ? 'var(--nav-active-bg)' : 'transparent', color: theme === 'dark' ? 'var(--nav-active-text)' : 'var(--text-muted)', transition: 'all 0.2s' }}><Moon size={12} /></div>
            </div>
          )}
        </div>
